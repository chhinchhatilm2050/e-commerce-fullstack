<script setup lang="ts">
  ...
  const handleSearchSubmit = (query: string) => {
    ...
    if (route.name === 'category' && currentSlug) {
      router.push({
        path: `/products/category/${currentSlug}`,
...
<template>
  <header
    class="sticky top-0 z-50 bg-white backdrop-blur-md sm:border-b border-gray-200"
        ...
        <div class="flex items-center gap-1 sm:gap-3">
          ...
          >
            <i
              ...
              class="ri-moon-line text-black text-lg"
            ></i>
            <i v-else class="ri-sun-line text-black dark:text-white"></i>
          </button>
          <button
            @click="handleToWishlist"
            class="btn-ghost w-8 h-8 flex items-center justify-center rounded-full dark:hover:bg-surface-100"
          >
            <i
              class="ri-poker-hearts-line text-black text-lg dark:text-white"
            ></i>
          </button>
          <button
            @click="cartOpen = true"
            class="relative btn-ghost w-8 h-8 p-0 flex items-center justify-center rounded-full dark:hover:bg-surface-100"
          >
            <i
