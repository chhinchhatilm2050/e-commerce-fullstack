<script setup lang="ts">
  ...
  const passwordStrength = computed<number>(() => {
    const p: string = registerForm.password
    if (!p) {
      return 0
    }
    const rules: RegExp[] = [
            ...
            <div class="bg-white dark:bg-surface-800 rounded-3xl">
                <form @submit.prevent="handleRegister" class="space-y-4">
                    <div class="flex flex-col">
                        <label class="label">{{ $t('register.gender') }} <span class="text-red-700">*</span></label>
                    ...
                    <div>
                        ...
                        <input v-model="registerForm.phoneNumber" type="email" class="input" :class="{'input-error': errors.phoneNumber}" :placeholder="$t('register.enterPhoneNumber')" />
