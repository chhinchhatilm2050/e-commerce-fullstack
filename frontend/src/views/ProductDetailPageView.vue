...
<template>
  <TopLoader
    :isLoading="
      ...
      cartStore.loading ||
      cartStore.getCartLoadingg ||
      cartStore.updateCartLoading ||
      cartStore.removeCartLoading
    "
  />
  <div v-if="productStore.loadingDetail" class="text-center py-20">
    <TopLoader
      :isLoading="productStore.loadingDetail || wishlistStore.loading"
    />
  </div>
      ...
      <div>
        <div class="flex items-baseline gap-2">
          <p
            ...
            :class="discountPercent ? 'text-red-700/90' : ''"
          >
        ...
        <div v-if="availableColors.length > 0" class="mt-6">
          <span class="text-sm font-medium block mb-2">
            Color:
            <span class="font-normal capitalize">{{ selectedColor }}</span>
          </span>
          <div class="flex flex-wrap gap-3">
            ...
            >
              ...
              <p class="text-black/90 dark:text-white/90 text-sm capitalize">
                {{ color }}
              </p>
            </div>
          </div>
        ...
        <div class="flex items-center gap-3 mt-6">
          ...
          >
            <i
              :class="
                product && wishlistStore.isWishlisted(product._id)
                  ? 'ri-heart-fill text-xl'
                  : 'ri-poker-hearts-line text-lg'
              "
            ></i>
          </button>
        </div>
        ...
        <div class="mt-8 border-t border-black/10 dark:border-white/20">
          <div class="border-black/10 dark:border-white/20 border-b">
            <button
              class="w-full flex items-center justify-between py-4 text-sm font-semibold"
            ...
            <p
              v-if="openSection === 'description'"
              class="pb-4 text-sm text-black/60 dark:text-white/60 leading-relaxed"
            >
              {{ product.description }}
            </p>
          </div>
          ...
          <div
            ...
            "
            class="border-b border-black/10 dark:border-white/20"
          >
            <button
              class="w-full flex items-center justify-between py-4 text-sm font-semibold"
              @click="toggleSection('specification')"
            >
              Specification
              <i
                :class="
